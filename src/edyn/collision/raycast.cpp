...
raycast_result raycast(entt::registry &registry, vector3 p0, vector3 p1) {
    ...
    auto raycast_shape = [&] (entt::entity entity) {
        auto sh_idx = index_view.get<edyn::shape_index>(entity);
        auto pos = origin_view.contains(entity) ? static_cast<vector3>(origin_view.get<edyn::origin>(entity)) : tr_view.get<position>(entity);
