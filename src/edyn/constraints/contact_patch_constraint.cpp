...
void contact_patch_constraint::prepare(entt::entity entity, constraint &con, 
    ...
    if (normal_row_entity == entt::null) {
        ...
        registry.assign<constraint_row>(normal_row_entity);
    ...
    auto rA = rotate(ornA_conj, patch_center - posA);

    auto patch_center_proj = patch_center - normal * dot(patch_center - pB, normal);
    auto rB = rotate(ornB_conj, patch_center_proj - posB);
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        auto tread_length = bristle_spacing;

        auto normal_pressure = normal_force / (tread_width * 2 * row_half_length * (1 - 0.25/2 - 0.25/3));

        auto row_half_arc = cyl.radius * std::acos(row_half_length / cyl.radius);
        ...
        patch_start = std::fmod(patch_start, perimeter);
        patch_end = std::fmod(patch_end, perimeter);
        ...
        for (size_t j = start_idx; j != end_idx; j = (j+1) % num_bristles) {
            ...
            if (dl2 > EDYN_EPSILON && force > friction_force) {
