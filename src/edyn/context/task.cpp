...
struct parallel_for_context {
    ...
    parallel_for_context(IndexType first_, IndexType last_, IndexType step_,
                         IndexType chunk_size_, size_t num_jobs, task_delegate_t task_)
        : current(first_)
        , last(last_)
        , step(step_)
...
struct parallel_for_async_context {
    ...
    parallel_for_async_context(IndexType first_, IndexType last_, IndexType step_,
                               IndexType chunk_size_, size_t num_jobs,
                               task_delegate_t task_, task_completion_delegate_t completion_)
        : current(first_)
        , first(first_)
        , last(last_)
        , step(step_)
