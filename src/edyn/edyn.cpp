...
static void init_meta() {
    ...
    std::apply([&](auto ... c) {
        (entt::meta_factory<decltype(c)>().template data<&decltype(c)::body, entt::as_ref_t>("body"_hs), ...);
    }, constraints_tuple);

    entt::meta_factory<null_constraint>().data<&null_constraint::body, entt::as_ref_t>("body"_hs);

    entt::meta_factory<entity_owner>()
        .data<&entity_owner::client_entity, entt::as_ref_t>("client_entity"_hs);

    entt::meta_factory<island_resident>()
        .data<&island_resident::island_entity, entt::as_ref_t>("island_entity"_hs);

    entt::meta_factory<parent_comp>()
