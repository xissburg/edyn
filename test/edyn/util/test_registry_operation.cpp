...
TEST(test_registry_operation, test_components) {
    ...
    entt::meta_factory<comp_with_entity>()
...
TEST(test_registry_operation, test_impl) {
    ...
    builder->replace_type_id(ent01, entt::type_id<another_comp>().hash());
    ops = builder->finish();
    ops.execute(reg1, emap);
    ...
    builder->remove_type_id(ent01, entt::type_id<another_comp>().hash());
    ops = builder->finish();
    ops.execute(reg1, emap);
