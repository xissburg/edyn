...
Not all inputs are idempotent. A steady steering input of a vehicle can be applied multiple times without changing the outcome. Now a _shift up_ input, which changes gear in a vehicle, is an input that will change the outcome if applied multiple times. Such inputs must be applied individually at the right time, as they're a form of _action_, not a state. If actions are required, the user must create a component that holds a list of timestamped actions, with a size that's kept under a limit, and tag it as networked. When received in the other end, the timestamps must be converted into local using the time delta from the clock synchronization process.
