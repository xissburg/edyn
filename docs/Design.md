...
The `edyn::registry_delta` also contains relevant island graph topology information which the `edyn::island_coordinator` can use to decide whether to split the island. This information includes the number of connected components, how many entities are in each of them, and the AABB of each connected component (maybe?). It is not necessary to split an island immediately after the number of connected components goes above one, since it might not be worth it if there's a great probability that the islands will merge together again or if splitting would not really benefit the simulation in terms of performance. It is necessary to experiment with splitting strategies. One possibility is to only split when there's enough separation between the AABBs of each connected component and there isn't much variance in the number of elements per connected component. Another possibility is to add a delay to the splitting attempt once the worker notifies the coordinator that the island could be split and push the delay forward if another topology information comes up before reaching the target time (aka _debounce_).

The main thread runs its own special broad-phase using `edyn::broadphase_main`, which is responsible for creating `edyn::contact_manifold`s between nodes that reside in different islands, thus creating the connection between islands since they are unaware of each other. These manifolds also have an associated `edyn::island_node` which references both rigid bodies. As new `edyn::contact_manifold`s are created, the `edyn::island_coordinator` merges islands since they now form a single connected component. The broad-phase uses a _dbvt_ (dynamic bounding volume tree) for the AABB of each island combined with a local dbvt for the AABBs of the entities in each island. First, it finds pairs of islands that have their AABBs intersecting, and then in these pairs it looks for entities in one island that intersect the AABB of an entity in the other island. 

When a physics component is modified, the changes have to be propagated to its respective `edyn::island_worker`. This can be done by assigning a `edyn::dirty` component to the changed entity and specifying which components have changed calling `edyn::dirty::updated<edyn::linvel, edyn::position>`, for example. An alternative is to call `edyn::world::refresh()` with the entity and components that need to be updated. It is also possible to connect `edyn::world::refresh` to the `entt::registry::on_update` signal for the desired component type so that every time it is replaced in the registry, a refresh will be automatically triggered.
