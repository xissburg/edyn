...
Each asset can be instantiated in its own way, by creating entities and assigning whatever components to them. Each entity to be created must have an internal id which is globally unique and identical in all machines. This id will be used to map internal ids to entity ids of rigid bodies and constraints that already exist in the other end.
...
It is important to assign the initial state of an asset right after instantiation. For that reason, an _asset sync_ is performed when the client has the asset ready to be instantiated. This will ask the server to send back the values of relevant components which represent the entities' state. That means synchronizing all components is not necessary which saves on data. All the other components are assumed to have the exact same values since they were instantiated using the same asset in the server. Only after receiving an _asset sync response_ can the client instantiate the asset and immediately apply the received registry snapshot which will override the asset state with the latest values.

When a body of an asset leaves the AABB of interest, the client is notified of all entities that left, not just the asset. All the client has to do is delete these entities locally. The process above will be repeated if they come back into the AABB of interest.
